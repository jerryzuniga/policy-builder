...
interface ManualData {
  ...
  financialCap: number; // ✅ keep numeric
  exclusions: string;
...
// ✅ Typed onChange (no any)
type OnChange = <K extends keyof ManualData>(field: K, value: ManualData[K]) => void;
...
const BRAND_COLOR = "#6C64DD";
const BRAND_COLOR_DARK = "#564FCC";
const LOGO_URL =
  "https://github.com/jerryzuniga/Preservation/blob/ce11cf49a7ae9130b7763e2de2d66cdcdf9d82ad/public/policy.png?raw=true";
...
const STEPS: Step[] = [
  ...
  { id: "screening", title: "Prioritization", icon: CheckSquare, description: "Intake and scoring matrix" },
...
const INITIAL_DATA: ManualData = {
  ...
  policyPackage: { exists: false, coveredTopics: {}, topicContent: {} },
  ...
  repairTypes: { critical: true, accessibility: false, energy: false, exterior: false },

  financialCap: 15000, // ✅ numeric only
  exclusions: "",

  intakeMethods: { phone: true, web: false, walkin: false },

  priorityFactors: { healthSafety: 5, lmiHouseholds: 3, olderAdults: 3 },
  ...
  kpis: { homesServed: true, avgCost: true, repairTimeline: false, clientSatisfaction: true, safetyIncidents: false },
  ...
  constructionActivities: { hasCatalog: null, eligibleScopes: "", ineligibleScopes: "", permitTriggers: "" },
...
/* =========================
   Persistence (✅ no side effects in setState)
========================= */

function normalizeLoadedData(raw: any): ManualData {
  const merged: ManualData = { ...INITIAL_DATA, ...(raw || {}) };

  // financialCap: enforce number
  const cap = (raw?.financialCap ?? merged.financialCap) as any;
  merged.financialCap = Number.isFinite(Number(cap)) ? Number(cap) : INITIAL_DATA.financialCap;

  // ensure nested objects exist
  merged.policyPackage = {
    exists: Boolean(raw?.policyPackage?.exists ?? merged.policyPackage.exists),
    coveredTopics: raw?.policyPackage?.coveredTopics ?? merged.policyPackage.coveredTopics ?? {},
    topicContent: raw?.policyPackage?.topicContent ?? merged.policyPackage.topicContent ?? {},
  };

  merged.clientServices = {
    stages: raw?.clientServices?.stages ?? merged.clientServices.stages,
    participation: {
      required: raw?.clientServices?.participation?.required ?? merged.clientServices.participation.required,
      options: raw?.clientServices?.participation?.options ?? merged.clientServices.participation.options,
      documentation: raw?.clientServices?.participation?.documentation ?? merged.clientServices.participation.documentation,
    },
  };

  merged.policyMap = raw?.policyMap ?? merged.policyMap;
  merged.procurement = {
    ...merged.procurement,
    ...(raw?.procurement ?? {}),
    requiredDocs: raw?.procurement?.requiredDocs ?? merged.procurement.requiredDocs,
  };

  merged.constructionActivities = { ...merged.constructionActivities, ...(raw?.constructionActivities ?? {}) };
  merged.sustainability = { ...merged.sustainability, ...(raw?.sustainability ?? {}) };
  merged.safety = { ...merged.safety, ...(raw?.safety ?? {}) };
  merged.volunteerStandards = { ...merged.volunteerStandards, ...(raw?.volunteerStandards ?? {}) };
  merged.projectFeasibility = { ...merged.projectFeasibility, ...(raw?.projectFeasibility ?? {}) };
  merged.governance = { ...merged.governance, ...(raw?.governance ?? {}) };
...
function computePolicy33Aligned(data: ManualData): boolean {
  // Conservative: require policy package + all required topics checked + at least some text + checklist all true
  const hasPolicy = data.policyPackage?.exists === true;
  const requiredTopicsOk = REQUIRED_TOPICS_2_1_1.every((t) => data.policyPackage?.coveredTopics?.[t.key] === true);
  const contentOk = REQUIRED_TOPICS_2_1_1.every((t) => {
    const txt = (data.policyPackage?.topicContent?.[t.key] || "").trim();
    return txt.length > 0;
  });
  const checklistOk = Object.values(data.policy33Checklist || {}).every(Boolean);
  const activitiesOk = data.constructionActivities?.hasCatalog === true;

  return hasPolicy && requiredTopicsOk && contentOk && checklistOk && activitiesOk;
}
...
const LandingPage: React.FC<{ onStart: () => void }> = ({ onStart }) => {
  return (
    <div className="min-h-screen bg-white">
          ...
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center space-x-2">
              ...
              <span className="text-xl font-bold text-gray-900 tracking-tight">
                Policy<span style={{ color: BRAND_COLOR }}>Builder</span>
              </span>
            </div>
            <button
              onClick={onStart}
      ...
      <div className="relative overflow-hidden pt-16 pb-24 lg:pt-32">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
          <div className="text-center max-w-3xl mx-auto">
            <div
              className="inline-flex items-center px-3 py-1 rounded-full text-[#3c366b] text-xs font-semibold uppercase tracking-wide mb-6"
            ...
            <p className="text-lg text-slate-600 mb-10 leading-relaxed">
              Stop starting from a blank page. The Policy Builder guides you through every step of creating a Board-ready
              Policy & Procedure manual, ensuring alignment with U.S. Policy 33.
            </p>
            <div className="flex flex-col sm:flex-row justify-center gap-4">
              ...
              >
                Launch Builder <ArrowRight className="ml-2 h-5 w-5" />
              </button>
              <a
                href="#how-it-works"
                className="inline-flex justify-center items-center px-8 py-4 border border-slate-200 text-lg font-semibold rounded-xl text-slate-700 bg-white hover:bg-slate-50 transition-all"
              >
                See How It Works
              </a>
            </div>
      ...
      <div id="how-it-works" className="py-24 bg-white border-t border-slate-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            <div>
              <h2 className="text-3xl font-bold text-slate-900 mb-6">A guided path to a better manual</h2>
              <p className="text-lg text-slate-600 mb-8">We broke manual writing into logical, bite-sized steps.</p>
            ...
            <div className="relative">
              <div className="absolute inset-0 bg-gradient-to-r from-[#6C64DD] to-purple-600 rounded-2xl transform rotate-3 opacity-20 blur-xl" />
      ...
      <div className="bg-slate-900 py-20">
        <div className="max-w-4xl mx-auto px-4 text-center">
          ...
          <p className="text-slate-400 mb-10 text-lg">Standardize operations and reduce risk.</p>
          <button
            onClick={onStart}
            ...
            style={{ backgroundColor: BRAND_COLOR }}
          >
            Launch Builder <ChevronRight className="ml-2 w-5 h-5" />
          </button>
        </div>
      </div>
    </div>
...
const GuidePanel: React.FC<{ stepId: string }> = ({ stepId }) => {
  const guideContent: Record<string, { title: string; text: string; link?: string; linkText?: string }> = {
    foundations: {
      ...
      text: "Providing accurate contact details ensures the exported manual is ready for distribution.",
    },
    policyMap: {
      title: "Governance vs. Operations",
      text: "Policy 33 requires specific topics in a written, board-approved policy. Use the Policy Package section to draft language.",
    },
    programModel: {
      title: "Roles & Responsibilities",
      text: "List roles, not people. Define who has signing authority to reduce ambiguity.",
    },
    scope: {
      title: "Pricing & Construction",
      text: "Be specific about what you do not do to manage expectations and risk.",
    },
    clientServices: {
      title: "Sweat Equity",
      text: "If participation is required, ensure accessible options and accommodations for households with limitations.",
    },
    screening: {
      title: "Project Selection",
      text: "Define feasibility and walk-away criteria to prevent scope creep and liability.",
    },
    lifecycle: { title: "Lifecycle", text: "Stages should align with Client Services so you have one source of truth." },
    workforce: {
      title: "Risk & Procurement",
      text: "List actual insurance minimums and training rules. This is a risk control, not a checkbox.",
    },
    performance: {
      title: "Sustainability",
      text: "Grant reporting often relies on the KPIs you select here.",
    },
    compliance: {
      title: "Automating Compliance",
      text: "This section verifies your alignment with U.S. Policy 33. Governance fields help generate a declaration.",
      link: "https://hfhi.sharepoint.com/sites/ComplianceRequirements/Shared%20Documents/Policy%2033_Home_Repairs.pdf?csf=1&e=ZJK1Q9&CID=46dbe841-b4a0-4ae3-8d69-22a34d8cc56a",
      linkText: "View Policy 33 PDF",
    },
    export: { title: "Ready for Approval", text: "Export generates a draft you can brand and sign." },
  };
      ...
      <div className="flex-1">
        <div className="flex items-center mb-4">
          <div className="p-2 rounded-full mr-3" style={{ backgroundColor: `${BRAND_COLOR}20` }}>
            {stepId === "clientServices" ? (
              <AlertCircle className="w-5 h-5" style={{ color: BRAND_COLOR }} />
            ) : (
              <Info className="w-5 h-5" style={{ color: BRAND_COLOR }} />
            )}
          </div>
        ...
        <h3 className="text-lg font-semibold mb-3" style={{ color: "#3c366b" }}>
          {content.title}
        </h3>
        ...
        {content.link && (
          <a
            ...
            className="flex items-center p-3 border rounded-lg text-sm font-medium transition-colors group"
            style={{
          ...
          >
            ...
            {content.linkText || "View Resource"}
...
const FoundationsStep: React.FC<StepProps> = ({ data, onChange }) => {
  const addStaff = () => {
    const newStaff: StaffMember = { id: Date.now(), name: "", title: "" };
    onChange("staff", [...(data.staff || []), newStaff]);
  };

  const updateStaff = (id: number, field: keyof StaffMember, value: string) => {
    ...
    <div className="space-y-8 max-w-4xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="md:col-span-2">
            <label className="block text-sm font-semibold text-gray-700 mb-2">
              Affiliate / Organization Name <span className="text-red-500">*</span>
            </label>
            <input
              type="text"
              className="w-full rounded-lg border-gray-300 shadow-sm p-2.5 border focus:border-[#6C64DD] focus:ring-[#6C64DD]"
              value={data.orgName}
          ...
          <div className="md:col-span-2">
            <label className="block text-sm font-semibold text-gray-700 mb-2">
              Service Area (Counties/Zips) <span className="text-red-500">*</span>
            </label>
            <input
              type="text"
              className="w-full rounded-lg border-gray-300 shadow-sm p-2.5 border focus:border-[#6C64DD] focus:ring-[#6C64DD]"
      ...
      <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
        <h4 className="font-bold text-gray-900 mb-4 flex items-center border-b pb-2">
          <User className="w-5 h-5 mr-2" style={{ color: BRAND_COLOR }} /> Key Staff Contacts
        </h4>
        <div className="space-y-3">
          {(data.staff || []).map((staff) => (
            <div key={staff.id} className="flex gap-3 items-center">
              <input
                type="text"
                className="flex-1 rounded-lg border-gray-300 shadow-sm p-2.5 border focus:border-[#6C64DD] focus:ring-[#6C64DD]"
                value={staff.name}
              ...
              />
              <button onClick={() => removeStaff(staff.id)} className="text-gray-400 hover:text-red-500">
                <Trash2 className="h-4 w-4" />
              </button>
            </div>
          ))}
          <button onClick={addStaff} className="text-sm font-medium flex items-center mt-2" style={{ color: BRAND_COLOR }}>
...
const PolicyMapStep: React.FC<StepProps> = ({ data, onChange }) => {
  const updateMap = (key: string, field: keyof PolicyMapEntry, value: PolicyMapEntry[keyof PolicyMapEntry]) => {
    const currentEntry = data.policyMap?.[key] || { org: false, program: false, programDetails: "" };
    onChange("policyMap", { ...data.policyMap, [key]: { ...currentEntry, [field]: value } });
  };

  const updatePackageExists = (exists: boolean) => {
    onChange("policyPackage", { ...data.policyPackage, exists });
  };

  const toggleCoveredTopic = (topicKey: string) => {
    const coveredTopics = { ...(data.policyPackage?.coveredTopics || {}) };
    coveredTopics[topicKey] = !coveredTopics[topicKey];
    onChange("policyPackage", { ...data.policyPackage, coveredTopics });
  };

  const updateTopicContent = (topicKey: string, value: string) => {
    const topicContent = { ...(data.policyPackage?.topicContent || {}) };
    topicContent[topicKey] = value;
    onChange("policyPackage", { ...data.policyPackage, topicContent });
  };
  ...
  return (
    <div className="space-y-8 max-w-4xl">
      <div className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm">
        <div className="bg-slate-50 px-6 py-4 border-b border-gray-200">
          <h3 className="font-bold text-gray-800 flex items-center">
            <FileText className="w-5 h-5 mr-2" style={{ color: BRAND_COLOR }} /> Repair Program Policy Package (2.1.1)
          </h3>
        </div>
        <div className="p-6">
          <p className="text-sm text-gray-600 mb-4">
            Policy 33 requires a board-approved policy covering specific topics. Select topics to draft language.
          </p>

          <label
            className={`flex items-center space-x-3 p-4 rounded-xl border transition-all cursor-pointer shadow-sm mb-6 ${
              data.policyPackage.exists ? "bg-[#6C64DD]/10 border-[#6C64DD]/20" : "bg-white border-gray-200"
            }`}
          >
            <input
              type="checkbox"
              ...
              checked={data.policyPackage.exists}
              onChange={(e) => updatePackageExists(e.target.checked)}
            />
                  ...
                  return (
                    <div
                      ...
                      className={`border rounded-lg p-3 transition-all ${
                        isChecked ? "bg-[#6C64DD]/10 border-[#6C64DD]/20" : "bg-white border-gray-200"
                      }`}
                    >
                      <label className="flex items-center space-x-3 cursor-pointer">
                        ...
                        />
                        <span className={`text-sm font-medium ${isChecked ? "text-[#6961CC]" : "text-gray-700"}`}>
                          {topic.label}
                        </span>
                      </label>
      ...
      <div className="border border-gray-200 rounded-xl overflow-hidden shadow-sm bg-white">
        <div className="bg-slate-50 px-6 py-4 border-b border-gray-200">
          <h3 className="font-bold text-gray-800 flex items-center">
            <Shield className="w-5 h-5 mr-2" style={{ color: BRAND_COLOR }} /> Affiliate Policy Map
          </h3>
        </div>

        <div className="p-6 bg-white border-b border-gray-100">
          <p className="text-sm text-gray-600">
            Use checkboxes to indicate where each policy exists. In some cases there may be both affiliate and program policy.
          </p>
        </div>
        ...
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-white">
            <tr>
              <th className="py-4 pl-6 pr-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider w-1/3">
                Category
              </th>
...
const ProgramModelStep: React.FC<StepProps> = ({ data, onChange }) => {
  ...
  const removeRole = (id: number) => onChange("roles", data.roles.filter((r) => r.id !== id));
  ...
  return (
    <div className="space-y-4">
        ...
        <div key={role.id} className="relative rounded-xl border border-gray-200 bg-white p-6 shadow-sm hover:shadow-md transition-shadow">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <input
                type="text"
                className="block w-full border-0 border-b-2 border-gray-100 focus:border-[#6C64DD] focus:ring-[#6C64DD] px-0 py-2 text-gray-900 font-medium placeholder-gray-300 transition-colors bg-transparent"
                value={role.title}
            ...
            <div>
              ...
              <input
                type="text"
                className="block w-full border-0 border-b-2 border-gray-100 focus:border-[#6C64DD] focus:ring-[#6C64DD] px-0 py-2 text-gray-900 placeholder-gray-300 transition-colors bg-transparent"
      ...
      <button
        onClick={addRole}
        className="w-full flex justify-center items-center py-4 border-2 border-dashed border-gray-300 rounded-xl text-sm font-semibold text-gray-500 hover:border-[#6C64DD] hover:text-[#6C64DD] hover:bg-[#6C64DD]/10 transition-all"
      >
...
const ScopeStep: React.FC<StepProps> = ({ data, onChange }) => {
  const updatePricing = (field: keyof ManualData["pricing"], value: string) =>
    ...
    <div className="space-y-8">
      <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
        <h4 className="font-bold text-gray-900 mb-4 flex items-center">
          <DollarSign className="w-5 h-5 mr-2" style={{ color: BRAND_COLOR }} /> Pricing & Repayment Model
        </h4>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <select
              ...
              value={data.pricing.modelType}
              onChange={(e) => updatePricing("modelType", e.target.value)}
            >
          ...
          <div>
            ...
            <input
              type="text"
              className="block w-full rounded-lg border-gray-300 p-2.5 border focus:border-[#6C64DD] focus:ring-[#6C64DD]"
              value={data.pricing.calculationMethod}
              onChange={(e) => updatePricing("calculationMethod", e.target.value)}
              placeholder="e.g. Materials + Labor + 15%"
            />
          </div>
          ...
          <div className="md:col-span-2">
            ...
            <textarea
              className="block w-full rounded-lg border-gray-300 p-2.5 border h-20 focus:border-[#6C64DD] focus:ring-[#6C64DD]"
              value={data.pricing.repaymentTerms}
              onChange={(e) => updatePricing("repaymentTerms", e.target.value)}
              placeholder="e.g. 0% interest, forgivable lien. Hardship deferrals available."
            />
          </div>
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => onChange("financialCap", Number(e.target.value || 0))}
              min={0}
            />
          </div>
        </div>
      ...
      <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
        <h4 className="font-bold text-gray-900 mb-4 flex items-center">
          <Hammer className="w-5 h-5 mr-2" style={{ color: BRAND_COLOR }} /> Construction Activities
        </h4>
            ...
            <div className="bg-blue-50 p-4 rounded-lg border border-blue-100 flex items-start">
              ...
              <div>
                ...
                <p className="text-sm text-blue-800">
                  Please attach <strong>"Appendix A: Construction Activities"</strong> to your final manual.
                </p>
              </div>
            </div>
          )}
            ...
            <div className="bg-amber-50 p-4 rounded-lg border border-amber-200 flex items-start">
              ...
              <div>
                ...
                <p className="text-sm text-amber-800">
                  To proceed compliant with Policy 33, you must define construction activities. Use the Catalog Builder to generate
                  this list.
                </p>
              </div>
            </div>
...
const ClientServicesStep: React.FC<StepProps> = ({ data, onChange }) => {
  const updateParticipation = (field: keyof ManualData["clientServices"]["participation"], value: string) => {
    onChange("clientServices", {
      ...data.clientServices,
      participation: { ...data.clientServices.participation, [field]: value },
    });
  };
  ...
  return (
    <div className="space-y-8">
      <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
        <h4 className="font-bold text-gray-900 mb-4 flex items-center">
          <User className="w-5 h-5 mr-2" style={{ color: BRAND_COLOR }} /> Owner Participation (Sweat Equity)
        </h4>

        <div className="grid grid-cols-1 gap-6">
          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-2">
              Participation Requirement <span className="text-red-500">*</span>
            </label>
            <select
              className="block w-full rounded-lg border-gray-300 p-2.5 border focus:border-[#6C64DD] focus:ring-[#6C64DD]"
          ...
          <div>
            ...
            <textarea
              className="block w-full rounded-lg border-gray-300 p-2.5 border h-20 focus:border-[#6C64DD] focus:ring-[#6C64DD]"
              value={participation.options}
              onChange={(e) => updateParticipation("options", e.target.value)}
              placeholder="e.g. Site prep, lunch, education classes. Physical limitations accommodated via..."
            />
          </div>
          ...
          <div>
            ...
            <input
              type="text"
              className="block w-full rounded-lg border-gray-300 p-2.5 border focus:border-[#6C64DD] focus:ring-[#6C64DD]"
              value={participation.documentation}
              onChange={(e) => updateParticipation("documentation", e.target.value)}
              placeholder="e.g. Homeowner Agreement Clause"
            />
          </div>
        </div>
      </div>
      ...
      <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
        <h4 className="font-bold text-gray-900 mb-4 flex items-center">
          <Activity className="w-5 h-5 mr-2" style={{ color: BRAND_COLOR }} /> Project Lifecycle Stages
        </h4>

        <div className="pl-4 border-l-2 border-gray-200 space-y-6">
          {stages.map((stage) => (
            <div key={stage.id} className="pl-4 relative">
              <div className="absolute -left-[21px] top-1 w-3 h-3 rounded-full" style={{ backgroundColor: BRAND_COLOR }} />
...
const ClientScreeningStep: React.FC<StepProps> = ({ data, onChange }) => {
  ...
  const toggleGroup = (key: string) => {
    ...
    if (newFactors[key] !== undefined) delete newFactors[key];
    else newFactors[key] = 3;
    onChange("priorityFactors", newFactors);
  };

  const updateFeasibility = (field: keyof ManualData["projectFeasibility"], value: string) => {
  ...
  return (
    <div className="space-y-8">
      <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
        <div className="flex items-center mb-4">
          <ClipboardCheck className="w-5 h-5 mr-2" style={{ color: BRAND_COLOR }} />
          <h4 className="font-bold text-gray-900">Project Assessment & Selection</h4>
        </div>
        <p className="text-sm text-gray-500 mb-6">Define how you assess feasibility and who makes the final decision.</p>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-2">Assessment Protocol</label>
            <select
              className="block w-full rounded-lg border-gray-300 shadow-sm p-2.5 border focus:border-[#6C64DD] focus:ring-[#6C64DD]"
              value={data.projectFeasibility.assessmentProtocol}
          ...
          <div className="md:col-span-2">
            ...
            <textarea
              className="block w-full rounded-lg border-gray-300 shadow-sm p-2.5 border h-24 focus:border-[#6C64DD] focus:ring-[#6C64DD]"
              value={data.projectFeasibility.feasibilityLimits}
              onChange={(e) => updateFeasibility("feasibilityLimits", e.target.value)}
              placeholder="Describe when a project is rejected (e.g. cost exceeds threshold, structural instability)."
            />
          ...
          {["phone", "web", "walkin"].map((channel) => (
            <label
              ...
              className={`flex items-center space-x-3 px-4 py-3 rounded-lg border cursor-pointer transition-all ${
                data.intakeMethods[channel]
                  ? "bg-[#6C64DD]/10 border-[#6C64DD] ring-1 ring-[#6C64DD]"
          ...
          {VULNERABLE_GROUPS.map((group) => (
            <label
              ...
              className={`flex items-start space-x-3 p-3 rounded-lg border cursor-pointer transition-all ${
                data.priorityFactors[group.key] !== undefined ? "bg-[#6C64DD]/10 border-[#6C64DD]" : "hover:bg-slate-50 border-transparent"
              }`}
            >
              <input
                type="checkbox"
              ...
              />
              <div>
                <div className={`text-sm font-medium ${data.priorityFactors[group.key] !== undefined ? "text-[#6961CC]" : "text-gray-700"}`}>
                  {group.label}
                </div>
          ...
          <div className="space-y-6">
            <div className="bg-slate-50 p-4 rounded-lg border border-slate-100">
              <div className="flex justify-between items-center mb-2">
                ...
                <span className="text-white text-xs font-bold px-2 py-1 rounded" style={{ backgroundColor: BRAND_COLOR }}>
                  {data.priorityFactors.healthSafety || 5}
                </span>
              </div>
              ...
              <input
                ...
                onChange={(e) =>
                  onChange("priorityFactors", { ...data.priorityFactors, healthSafety: parseInt(e.target.value, 10) })
                }
              />
            ...
            {Object.keys(data.priorityFactors)
              .filter((k) => k !== "healthSafety")
              .map((factor) => (
                <div key={factor}>
                  <div className="flex justify-between items-center mb-2">
                    <span className="text-sm font-medium text-gray-700">{getLabel(factor)}</span>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      onChange("priorityFactors", { ...data.priorityFactors, [factor]: parseInt(e.target.value, 10) })
                    }
                  />
                </div>
              ))}
...
const LifecycleStep: React.FC<StepProps> = ({ data }) => {
    ...
    <div className="space-y-8">
      <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
        ...
        <div className="p-4 bg-gray-50 rounded-lg">
          <p className="text-sm text-gray-600 italic">Lifecycle details are configured in "Client Services". This view is for reference.</p>
        </div>
      </div>
...
const WorkforceStep: React.FC<StepProps> = ({ data, onChange }) => {
  const updateProcurement = (field: keyof ManualData["procurement"], value: any) =>
    onChange("procurement", { ...data.procurement, [field]: value });

  const updateVolunteers = (field: keyof ManualData["volunteerStandards"], value: string) =>
    onChange("volunteerStandards", { ...data.volunteerStandards, [field]: value });

  const updateSafety = (field: keyof ManualData["safety"], value: string) => onChange("safety", { ...data.safety, [field]: value });
  ...
  return (
    <div className="space-y-8">
      <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
        <h4 className="font-bold text-gray-900 mb-4 flex items-center">
          <Briefcase className="w-5 h-5 mr-2" style={{ color: BRAND_COLOR }} /> Repair Partner (Contractor) Selection
        </h4>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <select
              className="block w-full rounded-lg border-gray-300 p-2.5 border focus:border-[#6C64DD] focus:ring-[#6C64DD]"
              value={data.procurement.selectionMethod}
              onChange={(e) => updateProcurement("selectionMethod", e.target.value)}
            >
          ...
          <div>
            ...
            <input
              type="text"
              className="block w-full rounded-lg border-gray-300 p-2.5 border focus:border-[#6C64DD] focus:ring-[#6C64DD]"
              value={data.procurement.minQualifications}
              onChange={(e) => updateProcurement("minQualifications", e.target.value)}
              placeholder="License, Insurance limits, etc."
            />
          </div>
            ...
            <div className="flex gap-4 flex-wrap">
              {["w9", "coi", "bonding", "warranty"].map((doc) => (
                <label key={doc} className="flex items-center space-x-2">
                  <input type="checkbox" checked={data.procurement.requiredDocs?.[doc] || false} onChange={() => toggleDoc(doc)} className="rounded focus:ring-[#6C64DD]" />
      ...
      <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
        <h4 className="font-bold text-gray-900 mb-4 flex items-center">
          <Users className="w-5 h-5 mr-2" style={{ color: BRAND_COLOR }} /> Volunteer Participation
        </h4>
        ...
        <div className="space-y-4">
          <div>
            ...
            <input
              type="text"
              className="block w-full rounded-lg border-gray-300 p-2.5 border focus:border-[#6C64DD] focus:ring-[#6C64DD]"
              value={data.volunteerStandards.allowedScopes}
              onChange={(e) => updateVolunteers("allowedScopes", e.target.value)}
            />
          </div>
          ...
          <div>
            ...
            <input
              type="text"
              className="block w-full rounded-lg border-gray-300 p-2.5 border focus:border-[#6C64DD] focus:ring-[#6C64DD]"
              value={data.volunteerStandards.supervision}
              onChange={(e) => updateVolunteers("supervision", e.target.value)}
            />
          </div>
          ...
          <div>
            ...
            <input
              type="text"
              className="block w-full rounded-lg border-gray-300 p-2.5 border focus:border-[#6C64DD] focus:ring-[#6C64DD]"
              value={data.volunteerStandards.training}
              onChange={(e) => updateVolunteers("training", e.target.value)}
            />
          </div>
        </div>
      </div>
      ...
      <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
        <h4 className="font-bold text-gray-900 mb-4 flex items-center">
          <HardHat className="w-5 h-5 mr-2" style={{ color: BRAND_COLOR }} /> Risk Management & Safety
        </h4>
        ...
        <div className="space-y-4">
          <div>
            ...
            <input
              type="text"
              className="block w-full rounded-lg border-gray-300 p-2.5 border focus:border-[#6C64DD] focus:ring-[#6C64DD]"
              value={data.safety.riskScreening}
              onChange={(e) => updateSafety("riskScreening", e.target.value)}
            />
          </div>
          ...
          <div>
            ...
            <input
              type="text"
              className="block w-full rounded-lg border-gray-300 p-2.5 border focus:border-[#6C64DD] focus:ring-[#6C64DD]"
              value={data.safety.specialtyContractorTriggers}
              onChange={(e) => updateSafety("specialtyContractorTriggers", e.target.value)}
            />
          </div>
        </div>
      </div>
...
const PerformanceStep: React.FC<StepProps> = ({ data, onChange }) => {
  const updateSustainability = (field: keyof ManualData["sustainability"], value: string) =>
    onChange("sustainability", { ...data.sustainability, [field]: value });

  return (
    <div className="space-y-8">
      <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
        <h4 className="font-bold text-gray-900 mb-4 flex items-center">
          <Activity className="w-5 h-5 mr-2" style={{ color: BRAND_COLOR }} /> Financial Sustainability
        </h4>
        ...
        <div className="grid grid-cols-1 gap-4">
          <div>
            ...
            <input
              type="text"
              className="block w-full rounded-lg border-gray-300 p-2.5 border focus:border-[#6C64DD] focus:ring-[#6C64DD]"
              value={data.sustainability.fundingMix}
              onChange={(e) => updateSustainability("fundingMix", e.target.value)}
            />
          </div>
          ...
          <div>
            ...
            <input
              type="text"
              className="block w-full rounded-lg border-gray-300 p-2.5 border focus:border-[#6C64DD] focus:ring-[#6C64DD]"
              value={data.sustainability.costControls}
              onChange={(e) => updateSustainability("costControls", e.target.value)}
            />
          </div>
          ...
          <div>
            ...
            <input
              type="text"
              className="block w-full rounded-lg border-gray-300 p-2.5 border focus:border-[#6C64DD] focus:ring-[#6C64DD]"
              value={data.sustainability.pipelineTargets}
              onChange={(e) => updateSustainability("pipelineTargets", e.target.value)}
            />
          </div>
        </div>
      </div>
...
const ComplianceStep: React.FC<StepProps> = ({ data, onChange }) => {
  const updateGovernance = (field: keyof ManualData["governance"], value: string) => {
    onChange("governance", { ...data.governance, [field]: value });
  };

  const toggleCheck = (key: string) => {
    onChange("policy33Checklist", { ...data.policy33Checklist, [key]: !data.policy33Checklist[key] });
  };
  ...
  return (
    <div className="space-y-8 max-w-4xl">
      <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
        <div className="bg-slate-50 px-6 py-4 border-b border-gray-200">
          <h3 className="font-bold text-gray-800">Compliance & Governance</h3>
        </div>

        <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-2">Approval Date</label>
            <input
              type="date"
              className="border p-2 rounded w-full"
              value={data.governance.approvalDate}
              onChange={(e) => updateGovernance("approvalDate", e.target.value)}
            />
          </div>

          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-2">Policy Version</label>
            <input
              type="text"
              className="border p-2 rounded w-full"
              value={data.governance.policyVersion}
              onChange={(e) => updateGovernance("policyVersion", e.target.value)}
              placeholder="Version"
            />
          </div>
        </div>
...
/* =========================
   Export (✅ escaped HTML + revokeObjectURL)
========================= */

const ExportStep: React.FC<{ data: ManualData }> = ({ data }) => {
  const policy33AlignedComputed = computePolicy33Aligned(data);
  ...
  const handleExport = () => {
    const safeOrgName = escapeHtml(data.orgName || "Repair Manual");
...
<head>
  ...
  <title>${safeOrgName} Repair Manual</title>
</head>
<body>
  <div class="cover-page">
    ...
    <div class="edition">
      ${safeOrgName}<br/>
      Version ${escapeHtml(data.governance.policyVersion || "1.0")}<br/>
      Date: ${escapeHtml(data.governance.approvalDate || new Date().toLocaleDateString())}
    </div>
  </div>

  <h1>1. Introduction & Foundations</h1>
  <p><strong>Affiliate:</strong> ${safeOrgName}</p>
  <p><strong>Address:</strong> ${escapeHtml(data.orgAddress || "N/A")}</p>
  <p><strong>Phone:</strong> ${escapeHtml(data.orgPhone || "N/A")} | <strong>Email:</strong> ${escapeHtml(data.orgEmail || "N/A")}</p>
  <p><strong>Service Area:</strong> ${escapeHtml(data.serviceArea || "N/A")}</p>

  <h3>Key Staff</h3>
  <ul>
    ${(data.staff || []).map((s) => `<li><strong>${escapeHtml(s.name || "")}</strong> - ${escapeHtml(s.title || "")}</li>`).join("")}
  </ul>

  <h2>2. Affiliate Policy Map</h2>
  <table>
    <tr><th>Category</th><th align="center">Org Level</th><th align="center">Program Level</th><th>Details</th></tr>
    ${Object.keys(data.policyMap || {})
      .map((key) => {
        const entry = data.policyMap[key];
        const detailsParts: string[] = [];
        if (entry?.org) detailsParts.push("Insert Affiliate-level Policy here.");
        if (entry?.program && entry?.programDetails) detailsParts.push(escapeHtml(entry.programDetails));
        return `
        <tr>
          <td>${escapeHtml(key.charAt(0).toUpperCase() + key.slice(1))}</td>
          <td align="center">${entry?.org ? "Yes" : ""}</td>
          <td align="center">${entry?.program ? "Yes" : ""}</td>
  ...
  ${REQUIRED_TOPICS_2_1_1.filter((topic) => data.policyPackage?.coveredTopics?.[topic.key]).map(
    (topic) => `
      <h3>${escapeHtml(topic.label)}</h3>
      <p>${nl2br(data.policyPackage?.topicContent?.[topic.key] || "No specific policy text provided.")}</p>
    `
  ).join("")}

  <h2>4. Pricing & Repayment</h2>
  <p><strong>Model:</strong> ${escapeHtml(data.pricing.modelType || "N/A")}</p>
  <p><strong>Calculation:</strong> ${escapeHtml(data.pricing.calculationMethod || "N/A")}</p>
  <p><strong>Terms:</strong> ${nl2br(data.pricing.repaymentTerms || "N/A")}</p>
  <p><strong>Financial Cap:</strong> $${escapeHtml(data.financialCap)}</p>

  <h2>5. Scope of Services</h2>
  <p><strong>Construction Activities:</strong> ${
    data.constructionActivities.hasCatalog ? "Refer to Appendix A: Construction Activities" : "Not Defined"
  }</p>
  <p><strong>Exclusions:</strong> ${nl2br(data.constructionActivities.ineligibleScopes || "N/A")}</p>
  <p><strong>Permits:</strong> ${nl2br(data.constructionActivities.permitTriggers || "N/A")}</p>

  <h2>6. Client Screening & Selection</h2>
  <h3>Intake</h3>
  <p><strong>Channels:</strong> ${
    Object.keys(data.intakeMethods || {})
      .filter((k) => data.intakeMethods[k])
      .map((k) => escapeHtml(k.charAt(0).toUpperCase() + k.slice(1)))
      .join(", ") || "None selected"
  }</p>

  <h3>Assessment & Selection</h3>
  <p><strong>Assessment Protocol:</strong> ${escapeHtml(data.projectFeasibility.assessmentProtocol || "N/A")}</p>
  <p><strong>Selection Authority:</strong> ${escapeHtml(data.projectFeasibility.selectionAuthority || "N/A")}</p>
  <p><strong>Decision Documentation:</strong> ${escapeHtml(data.projectFeasibility.selectionArtifact || "N/A")}</p>
  <p><strong>Feasibility Limits:</strong> ${nl2br(data.projectFeasibility.feasibilityLimits || "N/A")}</p>

  <h3>Prioritization</h3>
  <ul>
    ${Object.keys(data.priorityFactors || {})
      .map((key) => `<li><strong>${escapeHtml(getLabel(key))}:</strong> Weight ${escapeHtml(data.priorityFactors[key])}</li>`)
      .join("")}
  </ul>

  <h2>7. Client Services & Participation</h2>
  <p><strong>Participation Requirement:</strong> ${
    data.clientServices.participation.required === "required"
      ? "Required"
      : data.clientServices.participation.required === "not_required"
        ? "Not Required (Recommended)"
        : "N/A"
  }</p>
  <p><strong>Options & Accommodations:</strong> ${nl2br(data.clientServices.participation.options || "N/A")}</p>
  <p><strong>Documentation Method:</strong> ${escapeHtml(data.clientServices.participation.documentation || "N/A")}</p>

  <h2>8. Project Lifecycle</h2>
  <ol>
    ${(data.clientServices.stages || []).map((s) => `<li><strong>${escapeHtml(s.name)}</strong> (Trigger: ${escapeHtml(s.reqDoc)})</li>`).join("")}
  </ol>

  <h2>9. Workforce & Safety</h2>
  <p><strong>Delivery Model:</strong> ${escapeHtml(data.model || "N/A")}</p>
  <p><strong>QC Frequency:</strong> ${escapeHtml(data.qcFrequency || "N/A")}</p>

  <h3>Contractors</h3>
  <p><strong>Selection Method:</strong> ${escapeHtml(data.procurement.selectionMethod || "N/A")}</p>
  <p><strong>Min Qualifications:</strong> ${escapeHtml(data.procurement.minQualifications || "N/A")}</p>
  <p><strong>Required Docs:</strong> ${
    Object.keys(data.procurement.requiredDocs || {})
      .filter((k) => data.procurement.requiredDocs[k])
      .map((k) => escapeHtml(k.toUpperCase()))
      .join(", ")
  }</p>

  <h3>Volunteers</h3>
  <p><strong>Allowed Scopes:</strong> ${nl2br(data.volunteerStandards.allowedScopes || "N/A")}</p>
  <p><strong>Supervision:</strong> ${nl2br(data.volunteerStandards.supervision || "N/A")}</p>
  <p><strong>Training:</strong> ${nl2br(data.volunteerStandards.training || "N/A")}</p>

  <h3>Safety</h3>
  <p><strong>Risk Screening:</strong> ${nl2br(data.safety.riskScreening || "N/A")}</p>
  <p><strong>Safety Plan:</strong> ${nl2br(data.safety.safetyPlan || "N/A")}</p>
  <p><strong>Specialty Contractor Triggers:</strong> ${nl2br(data.safety.specialtyContractorTriggers || "N/A")}</p>

  <h2>10. Sustainability & Performance</h2>
  <p><strong>Funding Mix:</strong> ${nl2br(data.sustainability.fundingMix || "N/A")}</p>
  <p><strong>Cost Controls:</strong> ${nl2br(data.sustainability.costControls || "N/A")}</p>
  <p><strong>Pipeline Targets:</strong> ${nl2br(data.sustainability.pipelineTargets || "N/A")}</p>
  <p><strong>Reporting Schedule:</strong> ${escapeHtml(data.reportingSchedule || "")}</p>
  <p><strong>Feedback Mechanism:</strong> ${nl2br(data.feedbackMechanism || "")}</p>

  <h2>11. Compliance Declaration</h2>
  <p><strong>Policy 33 Alignment:</strong> ${policy33AlignedComputed ? "Compliant" : "Pending"}</p>
  <p><strong>Governance:</strong> Approved by ${escapeHtml(data.governance.approverRole)} on ${escapeHtml(data.governance.approvalDate || "N/A")}.</p>
</body>
</html>
`.trim();
    ...
    link.href = url;
    const name = (data.orgName || "Repair_Manual").replace(/\s+/g, "_");
    link.download = `Repair_Manual_${name}_Draft.doc`;
    ...
    <div className="flex flex-col items-center justify-center h-full py-12">
      <div className="bg-white p-10 rounded-2xl shadow-sm border border-gray-200 text-center max-w-lg w-full">
        ...
        <p className="text-gray-500 mb-8 leading-relaxed">
          Export a draft you can brand and sign. Text is escaped to prevent broken exports and injection.
        </p>
        <button
          onClick={handleExport}
          className="w-full flex justify-center items-center px-6 py-4 border border-transparent text-base font-bold rounded-xl shadow-lg text-white bg-blue-600 hover:bg-blue-700 transition-all"
        >
          <Download className="mr-2 h-5 w-5" /> Export to Word (.doc)
        </button>
      </div>
    </div>
  );
...
/* =========================
   Main App (✅ debounced localStorage save)
========================= */
...
export default function RepairManualBuilder() {
  ...
  useEffect(() => {
    ...
    if (localData) {
      try {
        ...
        setManualData(normalizeLoadedData(parsed));
      } catch (e) {
  ...
  // ✅ debounced save (no side effects inside setState)
  ...
  useEffect(() => {
    ...
    saveTimer.current = window.setTimeout(() => {
      try {
        localStorage.setItem(STORAGE_KEY, JSON.stringify({ ...manualData, lastUpdated: new Date().toISOString(), version: APP_VERSION }));
      } catch (e) {
  ...
  const handleDataChange: OnChange = useCallback((field, value) => {
    setManualData((prev) => ({ ...prev, [field]: value, lastUpdated: new Date().toISOString(), version: APP_VERSION }));
  }, []);

  const renderStepContent = () => {
    const commonProps: StepProps = { data: manualData, onChange: handleDataChange };

    switch (STEPS[currentStep].id) {
      case "foundations":
        return <FoundationsStep {...commonProps} />;
      case "policyMap":
        return <PolicyMapStep {...commonProps} />;
      case "programModel":
        return <ProgramModelStep {...commonProps} />;
      case "scope":
        return <ScopeStep {...commonProps} />;
      case "clientServices":
        return <ClientServicesStep {...commonProps} />;
      case "screening":
        return <ClientScreeningStep {...commonProps} />;
      case "lifecycle":
        return <LifecycleStep {...commonProps} />;
      case "workforce":
        return <WorkforceStep {...commonProps} />;
      case "performance":
        return <PerformanceStep {...commonProps} />;
      case "compliance":
        return <ComplianceStep {...commonProps} />;
      case "export":
        return <ExportStep data={manualData} />;
      default:
        return <div>Unknown Step</div>;
    }
  };

  const isStepComplete = (stepId: string, d: ManualData) => {
    if (stepId === "scope") return d.constructionActivities.hasCatalog === true;
    if (stepId === "foundations") return !!(d.orgName && d.orgAddress && d.orgPhone && d.orgEmail && d.serviceArea);
    if (stepId === "policyMap") {
      const categories = Object.values(d.policyMap || {});
      return categories.length > 0 && categories.every((cat) => cat.org || cat.program);
    }
    if (stepId === "clientServices") return d.clientServices.participation.required !== "";
    return false;
  };

  const isStepWarning = (stepId: string, d: ManualData) => {
    if (stepId === "scope") return d.constructionActivities.hasCatalog === false;
    if (stepId === "foundations") return !(d.orgName && d.orgAddress && d.orgPhone && d.orgEmail && d.serviceArea);
    if (stepId === "policyMap") {
      const categories = Object.values(d.policyMap || {});
      return categories.length === 0 || !categories.every((cat) => cat.org || cat.program);
